...
def build_cnn(num_classes: int, image_dim: int = 32):
    ...
    cnn_model = tf.keras.Sequential([
        # 1. conv2d
        tf.keras.layers.Conv2D(filters = 8, kernel_size = 3, strides = (1,1), padding = "same", input_shape = (250, 250, 3)),
        ...
        # 7. conv2d 
        tf.keras.layers.Conv2D(filters = 16, kernel_size = 3, strides = (1, 1), padding = "same"),
        ...
        # goal is to give some random rotations so that images of different orientations can all be perceived 
        tf.keras.layers.RandomRotation(
            factor=0.1,
            fill_mode='reflect',
            interpolation='bilinear',
        ...
        # can adjust these, but check out the model summary to make sure you know what you're doing...
        # know I'm missing an embedding layer here, but to what? 
        # tf.keras.layers.SpatialDropout1D(0.3),
        tf.keras.layers.LSTM(256, dropout = 0.3, recurrent_dropout = 0.3, return_sequences = False),
    ...
    plot_model(cnn_model, to_file='model_plot_paperLSTM.png', show_shapes=True, show_layer_names=True)
    return cnn_model

def train_cnn(X_train, X_test, Y_train, Y_test, model, epochs=100, batch_size=100):
...
if __name__ == "__main__":
   # TODO: fix the data loader, it doesn't actually filter stuff out with fewer than n images
    ...
    # seems like the dims need to be expanded here: 
